...
export default function App() {
  ...
  const [completeBusId, setCompleteBusId] = useState<string>('');
  ...
  const [busStats, setBusStats] = useState<Record<string, RouteStats>>({});
 ...
 const totalPassengers = useMemo(() => {
  ...
  if (file instanceof File) {
    formData.append(nodeId, file);
      ...
      try {
          const response = await fetch("http://localhost:5000/count_people", {
  ...
  const handleSimulationUpdate = (stats: Record<string, RouteStats>) => {
      setBusStats(stats);
  };
  ...
  const handleCompleteTrip = async () => {
    ...
    if (!completeBusId.trim()) {
        setToastMessage("Please enter Bus ID");
        return;
    }
    ...
    try {
        ...
        const response = await fetch(
            "http://localhost:5000/complete_trip",
            {
                method: "POST",
                headers: {
        ...
        setToastMessage(
            `Trip ${data.trip_id} completed successfully`
        );
        ...
        setToastMessage(
            error.message || "Trip completion failed"
        );
    }
};
...
<div className="col-span-2 bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center gap-3">

    <input
        type="text"
        placeholder="Enter Bus ID (e.g. B001)"
        value={completeBusId}
        onChange={(e) => setCompleteBusId(e.target.value)}
        className="
            flex-1
            px-4 py-3
            rounded-lg
