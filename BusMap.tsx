...
export const BusMap: React.FC<BusMapProps> = ({
  ...
  const completedCountsRef = useRef<Record<string, number>>({}); // Track completed buses by route ID
  
  // Schedule: Time (second) -> Array of Route IDs to dispatch
  const [dispatchSchedule, setDispatchSchedule] = useState<Record<number, string[]>>({});
  ...
  const handleMultiBusAnimation = (time: number) => {
    ...
    let busesFinishedThisFrame: string[] = [];
    ...
    ongoingBuses.forEach(bus => {
        ...
        const laneWidth = 14; 
        const numericRouteId = parseInt(bus.routeId.replace("R", ""));
        const laneIndex = (numericRouteId - 1) % 4; // 0, 1, 2, 3
